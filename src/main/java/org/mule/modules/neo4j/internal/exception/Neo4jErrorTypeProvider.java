...
public class Neo4jErrorTypeProvider implements ErrorTypeProvider {
	...
	public Set<ErrorTypeDefinition> getErrorTypes() {
		return ImmutableSet.<ErrorTypeDefinition> builder().add(CONNECTIVITY).add(REQUEST_FAILED)
				.add(INCORRECT_CREDENTIALS).add(UNKNOWN_HOST).add(INVALID_SESSION).add(UNKNOWN).build();
	}
