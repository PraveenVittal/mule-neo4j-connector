...
For hardware and software requirements, see the link:/release-notes/neo4jconnector[Neo4J Connector Release Notes].
...
NOTE: Neo4J's connector basic operations can filter nodes by only one label in its query. If you want to filter in a query by multiple labels, please use Execute operation with a proper CQL statement. A multi-label query example is provided  link:#use-case-2[here]
